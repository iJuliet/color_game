...
var ColorBox, MenuBox, React;

ColorBox = require("./colorBox.cjsx").ColorBox;
...
MenuBox = React.createClass({
  ...
  render: function() {
    var content, _ref;
    if ((_ref = this.state) != null ? _ref.hasStarted : void 0) {
      content = React.createElement(ColorBox, null);
    } else {
...
},{"./colorBox.cjsx":149,"react":147}],149:[function(require,module,exports){
var COLORS, ColorBox, MAXINDEX, React, TOTAL, colorSquare, colorsArray;
...
ColorBox = React.createClass({
  getInitialState: function() {
    var col, row, _i, _j;
    for (row = _i = 0; 0 <= MAXINDEX ? _i <= MAXINDEX : _i >= MAXINDEX; row = 0 <= MAXINDEX ? ++_i : --_i) {
      for (col = _j = 0; 0 <= MAXINDEX ? _j <= MAXINDEX : _j >= MAXINDEX; col = 0 <= MAXINDEX ? ++_j : --_j) {
  ...
  componentDidMount: function() {
    ...
    colorsArray[0].hasBeenCovered = true;
    this.setState({
      coveredAmount: 0,
      colors: colorsArray,
      surroundingIndex: this.state.surroundingIndex.push(0)
    });
    return this.updateColor(colorsArray[0].color);
  },
  updateSurroundings: function(selectedColor, previousSurroundings, coveredAmount) {
    var col, covered, downIndex, hasUpdated, i, leftIndex, rightIndex, row, selectedIndex, surroundings, upIndex;
    ...
    while (i < previousSurroundings.length) {
      ...
      if (colorsArray[selectedIndex].color === selectedColor) {
        ...
        if ((row - 1) >= 0 && !colorsArray[upIndex].hasBeenCovered && surroundings.indexOf(upIndex) === -1) {
          surroundings.push(upIndex);
          console.log(upIndex);
        }
        if ((row + 1) <= MAXINDEX && !colorsArray[downIndex].hasBeenCovered && surroundings.indexOf(downIndex) === -1) {
          surroundings.push(downIndex);
          console.log(downIndex);
        }
        if ((col - 1) >= 0 && !colorsArray[leftIndex].hasBeenCovered && surroundings.indexOf(leftIndex) === -1) {
          surroundings.push(leftIndex);
          console.log(leftIndex);
        }
        if ((col + 1) <= MAXINDEX && !colorsArray[rightIndex].hasBeenCovered && surroundings.indexOf(rightIndex) === -1) {
    ...
    if (hasUpdated) {
      return this.updateSurroundings(selectedColor, surroundings, covered);
    } else {
      if (covered === TOTAL) {
  ...
  updateColor: function(selectedColor) {
    var i;
    i = 0;
    while (i < TOTAL) {
      if (this.state.colors[i].hasBeenCovered) {
        colorsArray[i].update(selectedColor);
      }
      i++;
    }
    return this.updateSurroundings(selectedColor, this.state.surroundingIndex, this.state.coveredAmount);
  },
  allSameColor: function() {
    var col, color, colorSquare, row, _i, _j;
    color = this.state.colors[0].color;
    for (row = _i = 0; 0 <= MAXINDEX ? _i <= MAXINDEX : _i >= MAXINDEX; row = 0 <= MAXINDEX ? ++_i : --_i) {
      for (col = _j = 0; 0 <= MAXINDEX ? _j <= MAXINDEX : _j >= MAXINDEX; col = 0 <= MAXINDEX ? ++_j : --_j) {
        colorSquare = this.state.colors[row * (MAXINDEX + 1) + col];
        if (colorSquare.color !== color) {
          return false;
        }
      }
    }
  ...
  preupdateSurroundings: function(row, col) {
    var downIndex, leftIndex, rightIndex, selectedColor, upIndex;
    selectedColor = this.state.colors[row * (MAXINDEX + 1) + col].color;
    ...
    upIndex = (row - 1) * (MAXINDEX + 1) + col;
    downIndex = (row + 1) * (MAXINDEX + 1) + col;
    if ((colorsArray[upIndex].hasBeenCovered || colorsArray[upIndex].shouldUpdate) && (colorsArray[downIndex].hasBeenCovered || colorsArray[downIndex].shouldUpdate) && (colorsArray[leftIndex].hasBeenCovered || colorsArray[leftIndex].shouldUpdate) && (colorsArray[rightIndex].hasBeenCovered || colorsArray[rightIndex].shouldUpdate)) {
      return;
    }
    if ((row - 1) >= 0 && colorsArray[upIndex].color === selectedColor) {
      colorsArray[upIndex].preupdate();
      this.preupdateSurroundings(row - 1, col);
    }
    if ((row + 1) <= 7 && colorsArray[downIndex].color === selectedColor) {
      colorsArray[downIndex].preupdate();
      this.preupdateSurroundings(row + 1, col);
    }
    if ((col - 1) >= 0 && colorsArray[leftIndex].color === selectedColor) {
      colorsArray[leftIndex].preupdate();
      this.preupdateSurroundings();
    }
    if ((col + 1) <= 7 && colorsArray[rightIndex].color === selectedColor) {
      colorsArray[rightIndex].preupdate();
    }
  },
  render: function() {
    var btn, col, color, colorButtons, index, onerow, onesquare, row, squares, wrappedRow, _i, _j, _k, _len, _ref, _ref1;
    squares = [];
    for (row = _i = 0; 0 <= MAXINDEX ? _i <= MAXINDEX : _i >= MAXINDEX; row = 0 <= MAXINDEX ? ++_i : --_i) {
      onerow = [];
      for (col = _j = 0; 0 <= MAXINDEX ? _j <= MAXINDEX : _j >= MAXINDEX; col = 0 <= MAXINDEX ? ++_j : --_j) {
        index = row * (MAXINDEX + 1) + col;
        color = (_ref = this.state) != null ? _ref.colors[index].color : void 0;
        onesquare = React.createElement("td", null, React.createElement("button", {
          "type": "button",
          "className": "btn square-btn color-btn[" + index + "] " + color + "-btn"
        }));
        onerow.push(onesquare);
      }
      wrappedRow = React.createElement("tr", null, onerow);
    ...
    for (_k = 0, _len = COLORS.length; _k < _len; _k++) {
      ...
      btn = React.createElement("button", {
        ...
        "onClick": this.updateColor.bind(this, color)
      }, color);
      colorButtons.push(btn);
    }
    return React.createElement("div", null, React.createElement("h1", null, "Color Game"), React.createElement("div", {
      "id": "color-box"
    }, React.createElement("span", null, React.createElement("p", null, "Click on one of the color buttons, it will cover"), React.createElement("table", {
      "border": "1"
    }, squares)), React.createElement("p", null, "Covered Amount: ", ((_ref1 = this.state) != null ? _ref1.coveredAmount : void 0)), React.createElement("div", {
      "class": "color-buttons"
    }, colorButtons)));
  }
});
...
colorSquare = (function() {
  function colorSquare() {
    ...
    this.hasBeenCovered = false;
    this.shouldUpdate = false;
  }

  colorSquare.prototype.update = function(color) {
    this.color = color;
    this.hasBeenCovered = true;
    return this.shouldUpdate = false;
  };

  colorSquare.prototype.preupdate = function(color) {
...
Game = (function() {
  ...
  Game.prototype.start = function() {
    var menu;
    menu = Menu();
    return React.render(menu, document.getElementById('content'));
  };
...
},{"./Menu.cjsx":148,"./colorBox.cjsx":149,"react":147}]},{},[150]);